    ...
    stages {
        ...
        stage('Deploy MySQL to DEV') {
              steps {
                     ...
                     sh "docker run --name mysql-8.0 --rm --network dev -v thanhhiep-mysql-data:/var/lib/mysql -p 3308:3306 -e MYSQL_ROOT_PASSWORD=${MYSQL_ROOT_LOGIN_PSW} -e MYSQL_DATABASE=db_example_jenkinsV2 -d mysql:8.0 "
                     sh 'sleep 20'
